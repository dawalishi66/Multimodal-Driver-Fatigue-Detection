"""Six-class-aware structural validator for DCPT video metadata."""
...
from driver_state.preprocessing.distraction_video.build_metadata import VIDEO_6C_CSV_FIELDS
...
def _check_feature(row: dict[str, str], root: Path, duration_s: float) -> float:
    ...
    if not isinstance(declared, list) or any(type(value) is not int for value in declared):
    ...
    if np.any(np.diff(arrays["time_s"]) <= 0):
        raise ValueError("time_s must be strictly increasing")
    supports = arrays["support_s"]
    if (
        np.any(supports[:, 0] < 0)
        or np.any(supports[:, 1] > duration_s + 1e-9)
        or np.any(supports[:, 1] <= supports[:, 0])
    ):
        raise ValueError("support_s must contain positive intervals inside the sample")
    if np.any(arrays["time_s"] < supports[:, 0]) or np.any(
        arrays["time_s"] >= supports[:, 1]
    ):
...
def validate_video_6c(
    ...
    *,
    ...
    report: dict[str, Any] = {
        "status": "FAIL",
        "scope": "distraction_video_6c",
        "schema_version": "0.2.0",
        "label_scheme": label_scheme.get("label_scheme"),
        "row_count": 0,
    ...
    try:
        with Path(metadata).open(encoding="utf-8-sig", newline="") as stream:
            ...
            missing = sorted(set(VIDEO_6C_CSV_FIELDS) - set(fields))
            if missing:
    ...
    for row_number, raw in enumerate(rows, start=2):
        if any(raw.get(name) is None for name in VIDEO_6C_CSV_FIELDS):
        ...
        try:
            ...
            if not SESSION_ID_RE.fullmatch(row["session_id"]):
                raise ValueError(
                    "session_id must mirror the audio module: P<subject>_<date>_<HHMM>_<SS>"
                )
            sources = (
                json.loads(row["source_file"])
            ...
            if not isinstance(sources, list) or not sources or any(
                ...
                raise ValueError("source_file must be a relative resource or JSON list")
            for source in sources:
                candidate = source.replace("\\", "/")
                if not candidate or Path(candidate).is_absolute() or ".." in candidate.split("/"):
                    raise ValueError("source_file entries must be relative paths")
            ...
            if row["modality"] != "video":
                raise ValueError("modality must be video")
            ...
            if not expected_split or row["split"] != expected_split:
                raise ValueError("split disagrees with the six-class subject splits")
            task_key = row["sample_id"][:2]
            if task_key not in SIX_CLASS_TASK_TO_ID:
                raise ValueError("sample_id task prefix is outside the six-class set")
            label_id = int(row["label_id"])
            if (
                label_id != SIX_CLASS_TASK_TO_ID[task_key]
                or row["label_class"] != SIX_CLASS_ID_TO_NAME[label_id]
            ):
                raise ValueError("label_id/class must match the six-class video scheme")
